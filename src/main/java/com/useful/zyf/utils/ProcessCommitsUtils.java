...
public class ProcessCommitsUtils {

    public static String getGithubCommits(String web, String name) throws IOException {
        JSONArray jsonarray;
        JSONObject jsonObj;
        String id = "";
        int successCount = 0;
        ...
        String route = "C:/Users/dell/Desktop/" +dir_name + "-commits1" +"/";
        ...
        for(int i=94;i<500;i++){
            ...
            for(int j=0;j<jsonarray.size();j++){
                ...
                id = jsonObj.get("sha").toString();                    //取id
                String commits_url = (String) jsonObj.get("url");
                URL U3 = new URL(commits_url);
                HttpURLConnection connection3 = (HttpURLConnection)U3.openConnection();
                connection3.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.11 Safari/537.36");
                ...
                in3.close();
                JSONObject jsonObj3 = JSONObject.fromObject(result3);
                JSONArray jsonarray3 =JSONArray.fromObject(jsonObj3.get("files"));


                Set<String> filenameSet = new HashSet<String>();

                for(int p=0;p<jsonarray3.size();p++){
                    JSONObject jsonObj5  = JSONObject.fromObject(jsonarray3.getString(p));
//                        System.out.println(jsonObj5.get("filename"));       //相关文件名
                ...
                BufferedWriter bw =new BufferedWriter(new FileWriter(route + id + ".txt"
                        , true));
